...
def test_annealing_schedule_and_empirical_formula():
...
def test_random_rotation_matrix_is_orthogonal_for_fixed_input():
...
def test_load_config_file_rebases_relative_paths(tmp_path):
...
def test_build_input_dict_prefers_first_dictionary_and_respects_defaults():
    def target(required, optional="default", later=None):
        return required, optional, later

    result = build_input_dict(target, [{"required": 1, "optional": "first"}, {"optional": "second", "later": 3}])

    assert result == {"required": 1, "optional": "first", "later": 3}
    with pytest.raises(ValueError):
        build_input_dict(target, [{"optional": "only"}], raise_on_fail=True)
